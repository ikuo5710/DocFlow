...
| PDF.js | 5.x | PDFレンダリング | Mozilla製で信頼性が高い、ブラウザ互換、オープンソース |
...
| 技術 | バージョン | 用途 | 選定理由 |
|------|-----------|------|----------|
| Vitest | 4.x | テストフレームワーク | 高速、TypeScriptネイティブ対応、Viteエコシステム |
...
```json
{
  "dependencies": {
    ...
    "pdfjs-dist": "5.x"             // 完全固定（react-pdf依存）
  },
  "devDependencies": {
    "typescript": "~5.9.0",         // パッチのみ許可
    "vitest": "^4.0.0"              // マイナーまで許可
  }
}
```
